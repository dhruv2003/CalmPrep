...
export default function ProfilePage() {
  ...
  const displayName = profile?.name || user?.displayName || 'Demo Student';
  const email = profile?.email || user?.email || 'demo@student.com';
  const guardianEmail = profile?.guardianEmail || 'Not added yet';
        ...
        <section className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-6">
          <div className="neo-card bg-white border-4 shadow-[6px_6px_0_0_#141414]">
            <h2 className="text-2xl font-black mb-5 flex items-center gap-2">
              <UserCircle /> Student account
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <ProfileField label="Name" value={displayName} icon={<Brain size={20} />} color="bg-[#b8f7d4]" />
              <ProfileField label="Email" value={email} icon={<Mail size={20} />} color="bg-[#ffe66d]" />
              <ProfileField label="Language" value={language.toUpperCase()} icon={<Languages size={20} />} color="bg-[#9ddcff]" />
              <ProfileField
                label="Mode"
                value={isDemoMode ? 'Demo mode' : 'Firebase account'}
          ...
          <div className="neo-card bg-[#7c5cff] text-white border-4 shadow-[6px_6px_0_0_#141414]">
            <h2 className="text-2xl font-black mb-5 flex items-center gap-2">
              <ShieldCheck /> Safety setup
            </h2>
            <div className="rounded-lg border-3 border-[#141414] bg-white text-[#141414] p-4 shadow-[4px_4px_0_0_#141414]">
              <p className="text-sm font-black uppercase tracking-wide opacity-70">Trusted guardian</p>
            ...
            <div className="mt-4 rounded-lg border-3 border-[#141414] bg-[#ffe66d] text-[#141414] p-4 shadow-[4px_4px_0_0_#141414]">
              <p className="text-sm font-black uppercase tracking-wide opacity-70">Safety alerts</p>
              <p className="mt-1 text-lg font-black">{guardianEnabled ? 'Enabled with consent' : 'Not enabled'}</p>
            </div>
          </div>
        </section>
